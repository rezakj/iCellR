...
ha = as.data.frame(cbind(unlist(KNN2),unlist(KNN1),unlist(KNN3)))
colnames(ha)<- c("from","to","weight")
### run igraph
message(paste("    Generating igraph data frame ..."))
g <- graph.data.frame(ha, directed=FALSE)
